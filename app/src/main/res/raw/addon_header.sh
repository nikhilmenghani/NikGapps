...
addToLog() {
  echo "$(date +%Y_%m_%d_%H_%M_%S): $1" >> $nikGappsAddonLogFile
}
...
addFileToLog() {
  ...
  if [ -f "$output_path" ]; then
    ...
    echo "$(date +%Y_%m_%d_%H_%M_%S): - $output_path copied, Size: $size" >> $nikGappsAddonLogFile
  else
    echo "$(date +%Y_%m_%d_%H_%M_%S): - $output_path does not exist" >> $nikGappsAddonLogFile
  fi
}
