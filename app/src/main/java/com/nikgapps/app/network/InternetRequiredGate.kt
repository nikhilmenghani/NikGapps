...
@Composable
fun InternetRequiredGate(content: @Composable () -> Unit) {
    ...
    LaunchedEffect(Unit) { checkInternet() }
        ...
        val callback = object : ConnectivityManager.NetworkCallback() {
            ...
            override fun onCapabilitiesChanged(network: Network, capabilities: NetworkCapabilities) {
                if (capabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_VALIDATED)) {
                    ...
                    checkJob?.cancel()
                    scope.launch { internetState = InternetState.OFFLINE }
                }
            }
    ...
    BackHandler(enabled = internetState != InternetState.ONLINE) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        if (internetState != InternetState.ONLINE) {
            OfflineScreen(
...
@Composable
private fun OfflineScreen(checking: Boolean, onRetry: () -> Unit, onOpenSettings: () -> Unit) {
